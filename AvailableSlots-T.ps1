		...
		foreach($session in $webData.sessions)
		{
			if($session.min_age_limit -eq $null -OR $session.min_age_limit -le $minAgeLimit)
			{
                if($pincodeChoice -eq "ALL")
                {
					...
					if($session.available_capacity_dose1 -eq 0){
					}
					else{
						write-host "Address: "$session.address"      Block: "$session.block_name",		 Pin Code: "$session.pincode",	" -nonewline     
						write-host "Available: "$session.available_capacity_dose1 -ForegroundColor White -BackgroundColor DarkGreen
